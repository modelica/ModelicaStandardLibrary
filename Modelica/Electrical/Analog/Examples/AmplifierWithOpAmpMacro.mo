...
model AmplifierWithOpAmpMacro "Simple Amplifier circuit which uses OpAmpMacro"
...
With the test circuit OpAmp_macro a time domain analysis of the example arrangement with a sinusoidal input voltage (12 V amplitude, frequency 1 kHz) using the operational amplifier model OpAmp_macro is carried out. The working voltages are 15 V and -15 V.
<p>
</P>
</P>
</HTML>"),
    Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100},{100,
        ...
        Text(
          ...
          lineColor={255,0,0},
          textString="%name")}));
  Modelica.Electrical.Analog.Basic.OpAmpMacro opAmp_macro 
...
equation
  connect(resistor.n, opAmp_macro.m) annotation (Line(
  ...
  connect(resistor.p, sineVoltage.p) annotation (Line(
      ...
      color={0,0,255},
      smooth=Smooth.None));
  connect(resistor1.p, opAmp_macro.m) annotation (Line(
  ...
  connect(sineVoltage.n, ground.p) annotation (Line(
      ...
      color={0,0,255},
      smooth=Smooth.None));
  connect(opAmp_macro.p, ground.p) annotation (Line(
  ...
  connect(resistor2.n, ground.p) annotation (Line(
      ...
      color={0,0,255},
      smooth=Smooth.None));
  connect(opAmp_macro.p_supply, constantVoltage.p) annotation (Line(
      points={{-11.93,20.29},{18.035,20.29},{18.035,20},{48,20}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(opAmp_macro.m_supply, constantVoltage1.p) annotation (Line(
  ...
  connect(constantVoltage1.n, ground.p) annotation (Line(
      ...
      color={0,0,255},
      smooth=Smooth.None));
  connect(opAmp_macro.outp, resistor2.p) annotation (Line(
      points={{2,10},{21,10},{21,-14}},
      color={0,0,255},
      smooth=Smooth.None));

end AmplifierWithOpAmpMacro;
