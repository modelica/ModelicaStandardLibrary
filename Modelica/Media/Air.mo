...
Specific enthalpy of moist air is computed from pressure, temperature and composition with X[1] as the total water mass fraction. The fog region is included for both, ice and liquid fog.
  ...
  algorithm 
    ...
    X_sat:=min(p_steam_sat*k_mair/max(100*Constants.eps, state.p - p_steam_sat)*(1 - state.X[
      Water]), 1.0);
    X_liquid :=max(state.X[Water] - X_sat, 0.0);
    X_steam  :=state.X[Water] - X_liquid;
    X_air    :=1 - state.X[Water];
    R_gas:= gasConstant_X({X_steam/(1-X_liquid), X_air/(1-X_liquid)});
    h       := specificEnthalpy(state);
  //{SingleGasNasa.h_Tlow(data=steam,  T=state.T, refChoice=3, h_off=46479.819+2501014.5),
     //             SingleGasNasa.h_Tlow(data=dryair, T=state.T, refChoice=3, h_off=25104.684)}*
     //  {X_steam, X_air}+enthalpyOfWater(state.T)*X_liquid;
    u := h-R_gas*state.T;
    //u := h_pTX(state.p,state.T,state.X) - gasConstant(state)*state.T;
  /*Katrin: The equation above was replaced because the gas constant was computed from the total 
water mass fraction which gives incorrect results in the fog region*/
      annotation (Documentation(info="<html>
Specific internal energy is determined from the thermodynamic state record, assuming that the liquid or solid water volume is negligible.
   ...
   redeclare function extends specificEntropy 
      "Return specific entropy from thermodynamic state record, only valid for phi<1" 
       annotation(Inline=false,smoothOrder=5,
        Documentation(info="<html>
Specific entropy is calculated from the thermodynamic state record, assuming ideal gas behavior and including entropy of mixing. Liquid or solid water is negleted, the entire water content X[1] is assumed to be in vapor state (relative humidity below 1.0).
</html>"));
    protected 
...
The Specific Helmholtz Energy is computed from the thermodynamic state record for moist air with a water content below saturation.
   ...
   redeclare function extends specificHeatCapacityCp 
     ...
     annotation(Inline=false,smoothOrder=5,
        Documentation(info="<html>
The specific heat capacity at constant pressure <b>cp</b> is computed from temperature and composition for a mixture of steam (X[1]) and dry air. All water is assumed to be in vapor state.
</html>"));
   algorithm 
  ...
  redeclare function extends specificHeatCapacityCv 
     ...
     annotation(Inline=false,smoothOrder=5,
        Documentation(info="<html>
The specific heat capacity at constant density <b>cv</b> is computed from temperature and composition for a mixture of steam (X[1]) and dry air. All water is assumed to be in vapor state.
</html>"));
  algorithm 
