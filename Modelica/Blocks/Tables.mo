...
<li><strong>Read</strong> from a <strong>file</strong> \"fileName\" where the matrix is stored as
    \"tableName\". CSV, Dymola MOS and MATLAB MAT-file formats are possible.
    (Both the limitations on the CSV format and the Dymola MOS format are described below).
    Files with extension .csv are read as CSV format.
    Files with extension .mat are read as MAT-file format.
    Files with any other extension are read as Dymola MOS format.
...
<p>
If tables are read from a Dymola MOS file, the file needs to have the
following structure:
</p>
<blockquote><pre>
#1
...
Dymola MOS files should either be ASCII or UTF-8 encoded, where UTF-8 encoded strings are only allowed in line comments and an optional UTF-8 BOM at the start of the file is ignored.
