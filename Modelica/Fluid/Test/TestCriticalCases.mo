...
pipe wall friction and compressible valve characteristics. A ValveCompressible defined with Utilities.regRoot2 for the treatment of flow reversal
(cf. Modelica_Fluid Stream beta3) leads to wrong initial values. It works with Utilities.regRoot and a smooth(0, ...) expression for flow reversal.
<p>
  ...
  model BranchingPipes1
    ...
    Sources.Boundary_pT source(nPorts=1,
      ...
      T=300) annotation (Placement(transformation(extent={{-100,0},{-88,12}},
            rotation=0)));
    Modelica_Fluid.Test.BaseClasses.LumpedPipe pipe1(
      redeclare package Medium = Medium,
      use_T_start=true,
      length=10,
      diameter=2.54e-2,
...
Simulation starts with both valves open. At t=1, valve 1 closes; at t=2 valve 2 closes, and the simulation fails.
    ...
    Sources.Boundary_pT sink(
      ...
      T=300)   annotation (Placement(transformation(extent={{74,-20},{62,-8}},
            rotation=0)));
    Modelica_Fluid.Test.BaseClasses.LumpedPipe pipe2(
      redeclare package Medium = Medium,
      use_T_start=true,
      length=10,
      diameter=2.54e-2,
    ...
    Modelica_Fluid.Test.BaseClasses.LumpedPipe pipe3(
      redeclare package Medium = Medium,
      use_T_start=true,
      length=10,
      diameter=2.54e-2,
...
Simulation starts with both valves open. At t=1, valve 1 closes; at t=2 valve 2 closes, and the simulation fails.
    ...
    inner Modelica_Fluid.System system 
                          annotation (Placement(transformation(extent={{-100,60},
              {-80,80}}, rotation=0)));
    Modelica_Fluid.Fittings.TeeJunctionIdeal splitter(
                                     redeclare package Medium = Medium) 
      annotation (Placement(transformation(
          origin={-43,6},
          extent={{-6,-7},{6,7}},
...
Simulation starts with both valves open. At t=1, valve 1 closes; at t=2 valve 2 closes, and the simulation fails.
  ...
  model BranchingPipes15
    ...
    Modelica_Fluid.Test.BaseClasses.LumpedPipe pipe1(
      ...
      use_T_start=true,
      redeclare model FlowModel = 
        Modelica_Fluid.Pipes.BaseClasses.FlowModels.TurbulentPipeFlow (   dp_small=10),
      length=10,
      diameter=2.54e-2,
      p_a_start=500000,
