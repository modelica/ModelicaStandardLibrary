...
or other flow models without storage, are directly connected.
  ...
  model DynamicPipe
    "Dynamic pipe model with storage of mass and energy - Obsolete model! Instead, use PipeHydraulic, PipeOnePhaseHT or PipeTwoPhaseHT according to the desired application"
...
<li><a href=\"modelica://Modelica.Fluid.Examples.IncompressibleFluidNetwork\">Examples.IncompressibleFluidNetwork</a>.</li>
  ...
  model PipeHydraulic "Dynamic pipe model with storage of mass and energy"
    ...
    // Wall heat transfer
    parameter Boolean useInputHeatFlow = false
      "= true to use input connector to define heat flow"
        annotation (Dialog(tab="Assumptions", group="Heat transfer"));

    final parameter Real[n] dxs = lengths/sum(lengths);
    Modelica.Blocks.Interfaces.RealInput Q_flow if useInputHeatFlow annotation (Placement(
          transformation(
          extent={{-20,-20},{20,20}},
          rotation=270,
          ...
          rotation=270,
          origin={1,52})));
    parameter Modelica.SIunits.HeatFlowRate Q_flow_set=0
                                             annotation(Dialog(tab="Assumptions", group="Heat transfer",enable=not useInputHeatFlow));
  protected
    Modelica.Blocks.Interfaces.RealInput Q_flow_internal;

  equation
    if not useInputHeatFlow then
      Q_flow_internal = Q_flow_set;
    end if;
    connect(Q_flow,Q_flow_internal);

    Qb_flows = dxs*Q_flow_internal;
    // Wb_flow = v*A*dpdx + v*F_fric
    //         = v*A*dpdx + v*A*flowModel.dp_fg - v*A*dp_grav
    if n == 1 or useLumpedPressure then
      Wb_flows = dxs * ((vs*dxs)*(crossAreas*dxs)*((port_b.p - port_a.p) + sum(flowModel.dps_fg) - system.g*(dheights*mediums.d)))*nParallel;
...
<li><a href=\"modelica://Modelica.Fluid.Examples.IncompressibleFluidNetwork\">Examples.IncompressibleFluidNetwork</a>.</li>
  ...
  extends Modelica.Fluid.Pipes.BaseClasses.BasePipeHT(
                     ...
                     final massDynamics=massEnergyDynamics,
                     final energyDynamics=massEnergyDynamics);
     final parameter Integer nFMLumped=if modelStructure==Types.ModelStructureReduced.a_v_b then 2 else 1;
     final parameter Integer iLumped=integer(n/2)+1
      "Index of control volume with representative state if useLumpedPressure"
       annotation(Evaluate=true);

    parameter Types.Dynamics massEnergyDynamics=system.massDynamics
      "Formulation of mass and energy balances"
  ...
  equation
    ...
    // Energy and mass balances
       if massEnergyDynamics == Dynamics.SteadyState then
       for i in 1:n loop
    ...
    // staggered grid discretization for flowModel, depending on modelStructure
      ...
      if modelStructure == Types.ModelStructureReduced.av_b then
        port_a.p = mediums[1].p;
        statesFM[1] = mediums[iLumped].state;
        statesFM[2] = state_b;
        m_flows[n+1] = flowModel.m_flows[1];
      elseif modelStructure == Types.ModelStructureReduced.a_vb then
  ...
  initial equation
    if massEnergyDynamics == Dynamics.FixedInitial then
    ...
    elseif massEnergyDynamics == Dynamics.SteadyStateInitial then
      if Medium.ThermoStates == IndependentVariables.ph or
         Medium.ThermoStates == IndependentVariables.phX then
    ...
    annotation (defaultComponentName="pipe",
  Documentation(info="<html>
<p>Model of a straight pipe with distributed mass, energy and momentum balances. It provides the complete mass and energy balance equations for one-dimensional fluid flow as formulated in <a href=\"Modelica://Modelica.Fluid.UsersGuide.ComponentDefinition.BalanceEquations\">UsersGuide.ComponentDefinition.BalanceEquations</a>. The momentum balance is always static; energy and mass balance are either both static or both dynamic.</p>
<p>The partial differential equations are treated with the finite volume method and a staggered grid scheme for momentum balances. The pipe is split into nNodes equally spaced segments along the flow path. The default value is nNodes=2. This results in two lumped mass and energy balances and one lumped momentum balance across the dynamic pipe. </p>
<p>Note that this generally leads to high-index DAEs for pressure states if dynamic pipes are directly connected to each other, or generally to models with storage exposing a thermodynamic state through the port. This may not be valid if the dynamic pipe is connected to a model with non-differentiable pressure, like a Sources.Boundary_pT with prescribed jumping pressure. The <code><b>modelStructure</b></code> can be configured as appropriate in such situations, in order to place a momentum balance between a pressure state of the pipe and a non-differentiable boundary condition. </p>
...
<p>The <code><b>HeatTransfer</b></code> component specifies the source term <code>Qb_flows</code> of the energy balance. The default component uses a constant coefficient for the heat transfer between the bulk flow and the segment boundaries exposed through the <code>heatPorts</code>. The <code>HeatTransfer</code> model is replaceable and can be exchanged with any model extended from <a href=\"Modelica://Modelica.Fluid.Pipes.BaseClasses.HeatTransfer.PartialFlowHeatTransfer\">BaseClasses.HeatTransfer.PartialFlowHeatTransfer</a>. </p>
<p>The intended use is as pipes in one-phase heat exchangers.</p>
</html>"),
  Icon(coordinateSystem(preserveAspectRatio=true,  extent={{-100,-100},{100,100}},
          grid={1,1}), graphics={
          Rectangle(
  ...
  extends Modelica.Fluid.Pipes.BaseClasses.BasePipeHT(
    ...
    final parameter Integer nFMDistributed=if modelStructure==Types.ModelStructureReduced.a_v_b then n+1 else if (modelStructure==Types.ModelStructureReduced.a_vb or modelStructure==Types.ModelStructureReduced.av_b) then n else n-1;
    final parameter Integer iLumped=integer(n/2)+1
      "Index of control volume with representative state if useLumpedPressure"
      annotation(Evaluate=true);
  ...
  equation
    ...
    // staggered grid discretization of geometry for flowModel, depending on modelStructure
      ...
      elseif modelStructure == Types.ModelStructureReduced.a_v_b then
        //nFM = n+1;
         pathLengths[1:n+1] = cat(1, {0.5*lengths[1]}, lengths[2:n], {0.5*lengths[n]});
         dheightsFM[1:n+1] = cat(1, {0.5*dheights[1]}, dheights[2:n], {0.5*dheights[n]});
...
<p>The <code><b>HeatTransfer</b></code> component specifies the source term <code>Qb_flows</code> of the energy balance. The default component uses a constant coefficient for the heat transfer between the bulk flow and the segment boundaries exposed through the <code>heatPorts</code>. The <code>HeatTransfer</code> model is replaceable and can be exchanged with any model extended from <a href=\"Modelica://Modelica.Fluid.Pipes.BaseClasses.HeatTransfer.PartialFlowHeatTransfer\">BaseClasses.HeatTransfer.PartialFlowHeatTransfer</a>. </p>
<p>The intended use is as pipe in two-phase exchangers.</p>
</html>"),
  Icon(coordinateSystem(preserveAspectRatio=true,  extent={{-100,-100},{100,100}},
          grid={1,1}), graphics={
          Rectangle(
...
<li><i>04 Mar 2006</i>
    ...
    /*** Partial Straight Pipe ***/
      extends Modelica.Fluid.Interfaces.PartialTwoPort(
        final port_a_exposesState = (modelStructure == ModelStructureReduced.av_b),
        final port_b_exposesState = (modelStructure == ModelStructureReduced.a_vb));
    ...
    //Partial Distritubed Volume
     replaceable package Medium =
      ...
      final parameter SI.Volume[n] fluidVolumes= {crossAreas[i]*lengths[i] for i in 1:n}*nParallel
    ...
    protected
      ...
      // Geometry parameters
    public
      parameter SI.Length[n] lengths = fill(length/n,n);

      parameter SI.Area[n] crossAreas = fill(crossArea, n)
        "cross flow areas of flow segments"
        annotation(Dialog(group="Geometry"));
      parameter SI.Length[n] dimensions = fill(4*crossArea/perimeter, n)
        "hydraulic diameters of flow segments"
        annotation(Dialog(group="Geometry"));
      parameter SI.Height[n] roughnesses= fill(roughness, n)
        "Average heights of surface asperities"
        annotation(Dialog(group="Geometry"));
      ...
      parameter ModelStructureReduced modelStructure=ModelStructureReduced.a_vb
        "Determines whether flow or volume models are present at the ports"
        annotation(Dialog(tab="Advanced"), Evaluate=true);

      parameter Integer nFM "number of flow models in flowModel";
    ...
    public
      ...
      replaceable model HeatTransfer =
        ...
        "Wall heat transfer"
          annotation (Dialog(tab="Assumptions", group="Heat transfer",enable=use_HeatTransfer),choicesAllMatching=true);
      Modelica.Fluid.Interfaces.HeatPorts_a[n] heatPorts if use_HeatTransfer
        annotation (Placement(transformation(extent={{-10,45},{10,65}}), iconTransformation(extent={{-30,36},
                {32,52}})));
