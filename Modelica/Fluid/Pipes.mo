      ...
      protected
          ...
          equation
            ...
            if use_Ib_flows then
              Ib_flows = {rhos[i]*(vs[i]/nParallel)*(vs[i]/nParallel)*crossAreas[i] - rhos[i+1]*(vs[i+1]/nParallel)*(vs[i+1]/nParallel)*crossAreas[i+1] for i in 1:n-1};
      ...
      model NominalLaminarFlow
        ...
        SI.Length[n-1] pathLengths_nominal=
          {(dp_nominal-g*dheights[i])*Modelica.Constants.pi*((dimensions[i]+dimensions[i+1])/2)^4*rhos_act[i]/(128*mus_act[i])/
           (m_flow_nominal) for i in 1:n-1} if show_Res;
