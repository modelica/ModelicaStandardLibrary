...
model TestWallFriction
  ...
  replaceable package Medium =
    ...
    "Medium in all components" annotation (choicesAllMatching = true);
  parameter SI.Length roughness = 0.025e-3;

  Modelica.Fluid.Sources.Boundary_pT ambient_a(redeclare package Medium = Medium,
    p=system.p_ambient,
    T=system.T_ambient,
    use_p_in=true,
    nPorts=5)
  ...
  ModelicaTest.Fluid.BaseClasses.WallFriction pipe3(
    length=1,
    diameter=0.1,
    redeclare package Medium = Medium,
    roughness=roughness,
    use_Re=true) annotation (Placement(transformation(extent={{0,-20},{20,0}})));
 ModelicaTest.Fluid.BaseClasses.WallFriction pipe4(
    length=1,
    diameter=0.1,
    from_dp=false,
    redeclare package Medium = Medium,
    roughness=roughness,
    use_Re=true) annotation (Placement(transformation(extent={{0,-50},{20,-30}})));

  inner Modelica.Fluid.System system(use_eps_Re=true)
    annotation (Placement(transformation(extent={{68,74},{88,94}})));
  Modelica.Fluid.Pipes.StaticPipe pressureLossPipe(
    redeclare package Medium = Medium,
    length=1,
    diameter=0.1,
    roughness=roughness,
...
equation
  connect(p_table.y, ambient_a.p_in) annotation (Line(points={{-59,50},{-52,50},
          {-52,58},{-40,58}}, color={0,0,127}));
  connect(ambient_a.ports[1], pipe1.port_a) annotation (Line(points={{-18,53.2},{-2,
  ...
  connect(ambient_a.ports[2], pipe2.port_a) annotation (Line(points={{-18,51.6},{-6,
  ...
  connect(ambient_a.ports[3], pipe3.port_a) annotation (Line(points={{-18,50},{-10,50},
          {-10,-10},{0,-10}}, color={0,127,255}));
  connect(ambient_a.ports[4], pipe4.port_a) annotation (Line(points={{-18,48.4},{-14,
