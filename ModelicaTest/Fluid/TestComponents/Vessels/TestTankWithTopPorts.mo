...
equation
  connect(massFlowRate.ports[1], tank.topPorts)
                                            annotation (Line(
      points={{-62,80},{-40,80},{-40,81},{-20,81}}, color={0,127,255}));
  connect(tank.ports[1], mFlow_9m.port_a) annotation (Line(
      points={{-23,39},{-0.5,39},{-0.5,40},{20,40}}, color={0,0,255}));
  connect(mFlow_9m.port_b, Boundary_fixed.ports[1])
                                                annotation (Line(
      points={{40,40},{60,40},{60,-77},{80,-77}}, color={0,127,255}));
  connect(tank.ports[2], mFlow_6m.port_a) annotation (Line(
      points={{-21,39},{0,39},{0,0},{20,0}}, color={0,0,255}));
  connect(mFlow_6m.port_b, Boundary_fixed.ports[2])
                                                annotation (Line(
      points={{40,0},{60,0},{60,-79},{80,-79}}, color={0,127,255}));
  connect(tank.ports[3], mFlow_4m.port_a) annotation (Line(
      points={{-19,39},{0,39},{0,-40},{20,-40}}, color={0,0,255}));
  connect(mFlow_4m.port_b, Boundary_fixed.ports[3])
                                                annotation (Line(
      points={{40,-40},{60,-40},{60,-81},{80,-81}}, color={0,127,255}));
  connect(tank.ports[4], mFlow_2m.port_a) annotation (Line(
      points={{-17,39},{0,39},{0,-80},{20,-80}}, color={0,0,255}));
