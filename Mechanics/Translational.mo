...
"),     Icon(coordinateSystem(
    ...
    model ElastoGap "1D translational spring damper combination with gap" 
      ...
      annotation (
        ...
        Documentation(info="<html>
<p>
A <i>linear translational spring damper combination that can lift off</i>.<br>
The component can be connected between a sliding mass and the housing (model Fixed), 
to describe the contact of a sliding mass with the housing.<br>
Please note that there is no hard stop due to the housing.</p>
...
"),     Diagram(coordinateSystem(
    ...
    partial model PartialCompliantWithRelativeStates 
      ...
      SI.Distance s_rel(start=0) 
        "Relative distance (= flange_b.s - flange_a.s)";
      Real s_rel_scaled(stateSelect=stateSelect) 
        "Relative distance scaled with s_nominal";
...
"),     Diagram(coordinateSystem(
    ...
    equation 
      s_rel = flange_b.s - flange_a.s;
      s_rel_scaled = s_rel/s_nominal;
      v_rel = s_nominal*der(s_rel_scaled);
      a_rel = der(v_rel);
      flange_b.f =  f;
      flange_a.f = -f;
...
"),     Diagram(coordinateSystem(preserveAspectRatio=true,  extent={{-100,-100},
    ...
    equation 
      s_a = flange_a.s - internalSupport.flange.s;
      s_b = flange_b.s - internalSupport.flange.s;
      ...
      connect(support, support) annotation (Line(
          points={{0,-100},{2,-100},{2,-100},{0,-100}},
          color={0,127,0},
          smooth=Smooth.None));
...
"),     Diagram(coordinateSystem(
    ...
    protected 
      ...
      InternalSupport internalSupport 
        annotation (Placement(transformation(extent={{-10,-90},{10,-70}})));
    equation 
      s = flange.s - internalSupport.flange.s;
      v = der(s);
      a = der(v);
      // force balance
...
"),     Icon(coordinateSystem(
    ...
    model Accelerate 
      "Forced movement of a flange according to an acceleration signal" 
       extends Translational.Interfaces.PartialSource(s(start=0, fixed=true), v(start=0, fixed=true));
