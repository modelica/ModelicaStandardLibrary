...
"));
    ...
    package Basic "base functions as described in IAWPS/IF97" 
      ...
      annotation (Documentation(info="<HTML><h4>Package description</h4>
          ...
          as described in the standards document <a href=\"IF97documentation/IF97.pdf\">IF97.pdf</a>. The code of these
...
"));
  ...
  algorithm 
    ...
    elseif (aux.region == 4) then
      ...
      if p < BaseIF97.data.PLIMIT4A then
        t1:= BaseIF97.Basic.tph1(p, h_liq);
        t2:= BaseIF97.Basic.tph2(p, h_vap);
        ...
        aux.T := BaseIF97.Basic.tsat(p); // how to avoid ?
        d_liq:= rhol_T(aux.T);
        d_vap:= rhov_T(aux.T);
  ...
  algorithm 
	...
	vap := Common.helmholtzToBoundaryProps(fv);
      ...
      aux.dpT := (vap.s - liq.s)*(if (liq.d <> vap.d) then liq.d*vap.d/(liq.d - vap.d)
